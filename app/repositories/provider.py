...
from app.dtos.provider import ProviderCreateDTO, ProviderUpdateDTO
...
class ProviderRepository(BaseRepository[Provider, ProviderCreateDTO, ProviderUpdateDTO]):
    def __init__(self):
        pass

    def _is_valid_provider_name(self, provider_name: str) -> bool:
        # YTEST: 계정 이름 유효성 검사 (MySQL DB, User 이름 규칙 및 예약어 방지)
        if not re.match(r"^[a-zA-Z0-9_]{3,30}$", provider_name):
            return False
        ...
        if provider_name.lower() in reserved_words:
            return False
        return True
    ...
    def _execute_provider_db_query(self, db_name: str, query: str, params: tuple = None, multi: bool = False):
        ...
        try:
            ...
            print(f"Error executing provider DB query for {db_name}: {e}")
            raise

    def create(self, obj_in: ProviderCreateDTO) -> Provider:
        # YTEST: 새 계정 생성 및 MySQL DB/사용자 할당
        if not self._is_valid_provider_name(obj_in.username):
            raise ValueError(f"Invalid or reserved provider name: {obj_in.username}")
        ...
        try:
            with MySQLConnection() as conn:
                if conn:
                    ...
                    if cursor.fetchone():
                        raise ValueError(f"Provider with username '{obj_in.username}' already exists (DB name taken).")
                    cursor.execute(f"SELECT User FROM mysql.user WHERE User = '{obj_in.username}'")
                    if cursor.fetchone():
                        raise ValueError(f"Provider with username '{obj_in.username}' already exists (DB user taken).")
        except Error as e:
        ...
        self._execute_provider_db_query(db_name, schemas_sql_content, multi=True)

        # YTEST: 트리거 및 이벤트 생성
        self._execute_provider_db_query(db_name, TriggerQueries.create_user_insert_log_trigger(db_name))
        self._execute_provider_db_query(db_name, EventQueries.create_daily_cleanup_event(db_name))

        # YTEST: 계정 정보 (users 테이블에) 초기 삽입
        insert_initial_data_query = "INSERT INTO users (worker_id, worker_enpw) VALUES (%s, %s);"
        self._execute_provider_db_query(db_name, insert_initial_data_query, params=(db_user, obj_in.email))

        return Provider(id=1, username=obj_in.username, password=obj_in.password, email=obj_in.email, created_at=datetime.now(), db_name=db_name, db_user=db_user, db_password=db_password)

    def get(self, id: int) -> Optional[Provider]:
        ...
        try:
            with MySQLConnection() as conn:
                ...
                if result:
                    logger.info(f"Provider found for worker_uid: {id}")
                    # DB에서 받은 딕셔너리로 Provider 모델 인스턴스를 생성하여 반환합니다.
                    return Provider(**result)
                else:
                    logger.warning(f"Provider not found for worker_uid: {id}")
                    return None
        except Error as e:
            logger.error(f"Error retrieving multiple providers: {e}", exc_info=True)
            raise RuntimeError(f"Failed to retrieve providers from database: {e}")

    def get_by_field(self, field: str, value: str) -> Optional[Provider]:
    ...
    def get_multi(self, skip: int = 0, limit: int = 100) -> List[Provider]:
        # YTEST: 여러 계정 정보 조회 (현재는 더미 데이터)
        # print("YTEST: Retrieving multiple providers (Not implemented in DB)")
        """
                데이터베이스에서 여러 계정 정보를 조회합니다. (페이지네이션 적용)
                """
        # logger.info(f"Retrieving multiple providers from DB with skip={skip}, limit={limit}")
        ...
        try:
            with MySQLConnection() as conn:
                ...
                for row in results:
                    # 각 row 딕셔너리를 Provider 모델 객체로 변환하여 리스트에 추가합니다.
                    providers.append(Provider(**row))

                logger.info(f"Successfully retrieved {len(providers)} providers from DB.")
    ...
    def update(self, id: int, obj_in: ProviderUpdateDTO) -> Optional[Provider]:
        """
        ...
        try:
            ...
            with MySQLConnection() as conn:
                ...
                if cursor.rowcount > 0:
                    logger.info(
                        f"Successfully updated provider for worker_uid: {id}. Fields: {list(update_data.keys())}")
                    # 업데이트된 최신 정보를 다시 조회하여 반환하는 것이 가장 확실한 방법입니다.
                    return self.get(id)
                else:
                    # WHERE 조건에 맞는 행이 없거나, 값이 변경되지 않은 경우
                    logger.warning(
                        f"Update query executed for worker_uid: {id}, but no rows were affected. The provider may not exist or the data was the same.")
                    # 계정이 존재하는지 확인 후 반환
                    return self.get(id)

        except Error as e:
            logger.error(f"Error updating provider for worker_uid {id}: {e}", exc_info=True)
            raise RuntimeError(f"Failed to update provider in database: {e}")
    ...
    def delete(self, id: int) -> bool:
        ...
        logger.info(f"Attempting to soft delete provider with worker_uid: {id}")
        ...
        try:
            with MySQLConnection() as conn:
                ...
                if rows_affected > 0:
                    logger.info(f"Successfully soft-deleted provider with worker_uid: {id}")
                    return True
                else:
                    logger.warning(f"Provider with worker_uid: {id} not found or already deleted. No rows affected.")
                    return False

        except Error as e:
            logger.error(f"Error deleting provider for worker_uid {id}: {e}", exc_info=True)
            # DB 에러가 발생하면 상위 계층에서 처리하도록 예외를 다시 발생시킵니다.
            raise RuntimeError(f"Failed to delete provider in database: {e}")